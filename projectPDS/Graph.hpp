...
// create a typedef for the Graph type
    ...
    template<typename G> void insertChildrenParents(G g, )
        {
            auto vs = boost::vertices(g);
            for (auto vit = vs.first; vit != vs.second; ++vit) {
                auto neighbors = boost::adjacent_vertices(*vit, g);
    ...
    std::vector<E> edgesToModify(std::vector<E> edgesBack, std::vector<E> edgesTree, G g, std::string name) {
        ...
        std::set_difference(edgesBack.begin(), edgesBack.end(), edgesTree.begin(), edgesTree.end(),
                            std::inserter(finalEdges, finalEdges.begin()), [g](auto a, auto b) {
