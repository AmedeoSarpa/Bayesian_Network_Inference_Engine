...
public :
    ...
    RealVector(int size)  {
        ...
        rv_data->size = size;
        rv_data->values = std::shared_ptr<double[]>(new double[size]);
    }
    RealVector(std::vector<double> input)  {
        rv_data = std::make_shared<RealVectorData>();
        rv_data->size = input.size();
        rv_data->values = std::shared_ptr<double[]>(new double[input.size()]);
    ...
    void operator()(int states) {
        ...
        rv_data->values = std::shared_ptr<double[]>(new double[rv_data->size]);
    }
